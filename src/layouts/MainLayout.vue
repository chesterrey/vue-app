...
<template>
  <main class="relative">
    ...
    <slot></slot>
    <nav
      class="fixed bottom-0 grid grid-cols-3 w-full min-h-20 border-t bg-white font-semibold"
    >
      <router-link
        to="/home"
        class="pt-2 pb-8 text-center"
      ...
      <router-link
        ...
        :class="{
          'text-primary':
            route.name === 'blocks' || route.name === 'block-detail',
        }"
      >
      ...
      <router-link
        to="/progress"
        class="pt-2 pb-8 text-center"
        :class="{ 'text-primary': route.name === 'progress' }"
      >
